...
def compare_line_rtt_by_isp(max_results,isp,country,earliest,latest,city):	
	result = []
	...
	if city!='none':
		devices = devices.filter(geoip_city=city)
	for d in devices:
...
def compare_line_lmrtt_by_isp(max_results,isp,country,earliest,latest,city):	
	result = []
	#all devices under given isp:
	devices = database_helper.get_details_by_isp(isp)
	if country!='none':
...
def compare_line_bitrate_by_isp(max_results,isp,country,direction,earliest,latest,city):
	result = []
	#all devices under given isp:
	devices = database_helper.get_details_by_isp(isp)
