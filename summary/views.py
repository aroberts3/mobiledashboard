...
def compare_line_bitrate_by_isp(request):	
	...
	earliest = datetime_helper.format_date_from_calendar(start)
	latest = datetime_helper.format_date_from_calendar(end)
	series = views_helper.compare_line_bitrate_by_isp(max_results,isp,country,direction,earliest,latest,city)
	return HttpResponse(json.dumps(series))
...
def compare_line_lmrtt_by_isp(request):	
	...
	earliest = datetime_helper.format_date_from_calendar(start)
	latest = datetime_helper.format_date_from_calendar(end)
	series = views_helper.compare_line_lmrtt_by_isp(max_results,isp,country,earliest,latest,city)
	return HttpResponse(json.dumps(series))
...
def compare_line_rtt_by_isp(request):	
	...
	earliest = datetime_helper.format_date_from_calendar(start)
	latest = datetime_helper.format_date_from_calendar(end)
	series = views_helper.compare_line_rtt_by_isp(max_results,isp,country,earliest,latest,city)
	return HttpResponse(json.dumps(series))
...
def device_summary(request):
	device = str(request.POST.get('device')).lstrip('OWow')
	device_details = database_helper.get_details_by_deviceid(device)
	try:
	...
	if(request.POST.get('edit')):
		try:
			...
			return render_to_response('invalid_edit.html', {'deviceid' : device.replace(':','').lower()})
	if (hashing==''):
