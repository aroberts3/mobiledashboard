...
def write_bitrate_country_averages():
	...
	SQL =  "SELECT \
			...
			variance(m_bitrate.average) AS variance \
			FROM m_bitrate \
	...
	for r in records:
		try:
			...
			if var == None:
				var = 0
			line = str(avg) + '|' + str(m_count) + '|' + str(day) + '|' + country + '|' + str(d_count) + '|' + dir + '|' + isp + '|' + str(var) + '\n'
			f.write(line)
		except:
			continue
	f.close()
...
def write_bitrate_city_averages():
	...
	SQL =  "SELECT \
			...
			count(*) AS nmeasurements, \
			avg(m_bitrate.average) AS bitrate, \
			variance(m_bitrate.average*1000) AS variance \
			FROM m_bitrate \
			JOIN devicedetails AS d ON d.deviceid = m_bitrate.deviceid \
	...
	for r in records:
		try:
			...
			if var==None:
				var=0
			line = str(avg) + '|' + str(m_count) + '|' + str(day) + '|' + city.encode('utf-8') + '|' + str(d_count) + '|' + dir + '|' + isp.encode('utf-8') + '|' + str(var) + '\n'
			f.write(line)
		except:
			continue
	f.close()
...
def write_bitrate_isp_averages():
	...
	for r in records:
		try:
			...
			if var==None:
				var=0
			line = str(avg) + '|' + str(m_count) + '|' + str(day) + '|' + isp.encode('utf-8') + '|' + str(d_count) + '|' + dir + '|' + country.encode('utf-8') + '|' + city.encode('utf-8') + '|' + str(var) + '\n'
			f.write(line)
		except:
			continue
	f.close()
